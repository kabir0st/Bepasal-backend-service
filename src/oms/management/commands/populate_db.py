...
from oms.models import (Category, VariationType,
                        VariationOption, Item, ItemVariation)
    ...
    def populate_items(self):
        ...
        for i in range(100):
            with transaction.atomic():
                ...
                item.categories.set(random.sample(
                    list(categories), random.randint(1, 5)))
                item.enabled_variation_types.set(random.sample(
                    list(variation_types), random.randint(1, 3)))

                for _ in range(8):
                    variation_option_combination = []
                    for variation_type in item.enabled_variation_types.filter(
                    ...
                    if variation_option_combination not in applied_variation_combinations:
                ...
                self.stdout.write(self.style.SUCCESS(
                    f'Successfully populated item: {item}'))
